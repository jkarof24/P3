...
# DIG / PyG explainers (ensure these packages are installed)
from torch_geometric.explain import Explainer, GNNExplainer, PGExplainer
from dig.xgraph.method.subgraphx import SubgraphX  # adjust import if your SubgraphX location differs
...
# GNN Model with flexible forward signature (accepts data= or x, edge_index, batch)
class GNN_Model(nn.Module):
    ...
    def forward(self, x=None, edge_index=None, batch=None, data=None, **kwargs):
    ...
    for model_name in ['GCN', 'GAT', 'GraphSAGE']:
        ...
        for name, explainer in explainers.items():
            ...
            for i, data in enumerate(test_loader):
                ...
                try:
                    if name == 'SubgraphX':
                        # SubgraphX returns (something, explanation_results, something)
                        ...
                        if isinstance(sub_out, dict):
                            sub_nodes = sub_out.get('coalition')
                        ...
                        raw_mask = getattr(explanation, 'edge_mask', None) or explanation.get('edge_mask')
                        if raw_mask is None:
